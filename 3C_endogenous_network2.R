...
#' 
#' This file replicates the peer effect model estimation controlling for network endogeneity after
#' removing completely isolated students.
...
keptStudent   <- sapply(StudentID, function(x) x %in% mydata$sqid)
...
festim        <- function(k, eff = "fe", interac = FALSE){
  data        <- readRDS(file = paste0("_output/endogeneity/data.np", k, ".RDS"))[keptStudent,]
  if(interac){
  ...
  tryCatch({# May return error because of multicollinearity, so we tryCatch
    siv.FE1   <- summary(iv.FE1, vcov = vcovCL, cluster = unlist(lapply(1:length(F1), function(x) rep(x, nrow(F1[[x]])))))
    ...
    saveRDS(c(siv.FE1[c("coefficients", "sigma", "r.squared","adj.r.squared", "vcov", "diagnostics")], endo.t1), 
            file = paste0("_output/endogeneity/ed.siviso.", eff, ifelse(interac, ".in", ".ad"), k,  ".FE1.RDS"))
  ...
  saveRDS(c(siv.FE2[c("r.squared","adj.r.squared", "diagnostics")], va.iv.FE2, endo.t2),
          file = paste0("_output/endogeneity/ed.va.iviso", eff, ifelse(interac, ".in", ".ad"), k,  ".FE2.RDS"))
}
...
va.iv.FE2    <- readRDS(file = "_output/va.iviso.FE2.RDS")
...
PE[c(59, 67, 75, 83), 2] <- c(0.730213, 7.331e-01, 0.738501, 7.406e-01) 
PE[c(59, 67, 75, 83), 3] <- PE[51, 3]
...
eff          <- "fe"
interac      <- TRUE
ED.siv.FE1   <- readRDS(file = paste0("_output/endogeneity/ed.siviso.", eff, ifelse(interac, ".in", ".ad"), k,  ".FE1.RDS"))
write.csv(ED.siv.FE1$coefficients, file = paste0("_output/ed.siviso.", eff, ifelse(interac, ".in", ".ad"), k,  ".FE1.csv"))
ED.siv.FE1$sigma^2
...
eff          <- "fe"
interac      <- TRUE
ED.iv.FE2    <- readRDS(file = paste0("_output/endogeneity/ed.va.iviso", eff, ifelse(interac, ".in", ".ad"), k,  ".FE2.RDS"))
write.csv(ED.iv.FE2$output, file = paste0("_output/ed.va.iviso", eff, ifelse(interac, ".in", ".ad"), k,  ".FE2.csv"))
ED.iv.FE2$output
ED.iv.FE2$sigma2eta
